...
def char_maps(text: str):
    ...
    # char_to_idx = dict.fromkeys(uniques, range(len(uniques)))

    char_to_idx = dict(zip(uniques, range(len(uniques))))
...
def remove_chars(text: str, chars_to_remove):
    ...
    n_removed = 0
    # ====== YOUR CODE: ======
    char_to_idx, idx_to_char = char_maps(text)
    for char_to_remove in chars_to_remove:
        if char_to_remove in char_to_idx.keys():
            n_removed += text_clean.count(char_to_remove)
            text_clean.replace(char_to_remove, "")
    # ========================
...
def chars_to_onehot(text: str, char_to_idx: dict) -> Tensor:
    ...
    # one_hotting = torch.ones_like((1,), dtype=torch.int8).item()
    result = torch.zeros((N, D), dtype=torch.int8)
    for i, char in enumerate(text):
        result[i, [char_to_idx[char]]] = 1
    result.to(torch.int8)
    # ========================
    return result
...
def onehot_to_chars(embedded_text: Tensor, idx_to_char: dict) -> str:
    ...
    # ====== YOUR CODE: ======
    chars = []
    for char_ in torch.unbind(embedded_text):
        chars.append(idx_to_char[torch.nonzero(char_).item()])
    result = "".join(chars)
    # ========================
    return result
...
def chars_to_labelled_samples(text: str, char_to_idx: dict, seq_len: int, device="cpu"):
    ...
    samples = torch.stack(torch.split(embed_text, seq_len, dim=0), dim=0)
    one_hot_labels = torch.roll(samples, shifts=(0, 1, 1), dims=samples.shape)
    labels = onehot_to_chars(one_hot_labels.reshape(embed_text.shape), char_maps(text)[1])
    # ========================
